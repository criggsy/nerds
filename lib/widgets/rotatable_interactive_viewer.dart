...
// Fork of Flutter's interactive_viewer.dart — rotation enabled.
// Re-sync with SDK when upgrading Flutter.
...
/// See also:
///
///   * [RotatableInteractiveViewer.builder], whose builder is of this type.
///   * [WidgetBuilder], which is similar, but takes no viewport.
typedef RotatableInteractiveViewerWidgetBuilder = Widget Function(BuildContext context, Quad viewport);
...
///
/// By default, RotatableInteractiveViewer clips its child using [Clip.hardEdge].
/// To prevent this behavior, consider setting [clipBehavior] to [Clip.none].
/// When [clipBehavior] is [Clip.none], RotatableInteractiveViewer may draw outside of
...
/// To prevent dead areas where RotatableInteractiveViewer does not receive gestures,
/// don't set [clipBehavior] or be sure that the RotatableInteractiveViewer widget is the
...
///     which includes the use of RotatableInteractiveViewer.
///   * The [flutter-go demo](https://github.com/justinmc/flutter-go), which includes robust positioning of an RotatableInteractiveViewer child
///     that works for all screen sizes and child sizes.
///   * The [Lazy Flutter Performance Session](https://www.youtube.com/watch?v=qax_nOpgz7E), which includes the use of an RotatableInteractiveViewer to
...
/// {@end-tool}
@immutable
class RotatableInteractiveViewer extends StatefulWidget {
  /// Create an RotatableInteractiveViewer.
  ...
  /// Creates an RotatableInteractiveViewer for a child that is created on demand.
  ///
  ...
  /// large child.
  RotatableInteractiveViewer.builder({
    super.key,
    this.clipBehavior = Clip.hardEdge,
    this.panAxis = PanAxis.free,
    this.boundaryMargin = EdgeInsets.zero,
    ...
    this.alignment,
    this.trackpadScrollCausesScale = false,
    required RotatableInteractiveViewerWidgetBuilder this.builder,
  }) : assert(minScale > 0),
       assert(interactionEndFrictionCoefficient > 0),
       assert(minScale.isFinite),
       assert(maxScale > 0),
  ...
  /// If set to [Clip.none], the child may extend beyond the size of the RotatableInteractiveViewer,
  /// but it will not receive gestures in these areas.
  /// Be sure that the RotatableInteractiveViewer is the desired size when using [Clip.none].
  ///
  ...
  ///
  /// Passed with the [RotatableInteractiveViewer.builder] constructor. Otherwise, the
  ...
  final RotatableInteractiveViewerWidgetBuilder? builder;

  /// The child [Widget] that is transformed by RotatableInteractiveViewer.
  ///
  /// If the [RotatableInteractiveViewer.builder] constructor is used, then this will be
  ...
  /// is often useful when a child should be bigger than the RotatableInteractiveViewer.
  ///
  ...
  /// If false, two-finger rotation is ignored and the scale [GestureDetector]
  /// may be omitted when combined with disabled pan/scale (see [panEnabled]).
  ///
  /// Defaults to true.
  ...
  ///
  /// {@template flutter.widgets.RotatableInteractiveViewer.onInteractionEnd}
  ...
  ///
  /// A [GestureDetector] wrapping the RotatableInteractiveViewer will not respond to
  ...
  ///
  /// {@macro flutter.widgets.RotatableInteractiveViewer.onInteractionEnd}
  ///
  /// The coordinates provided in the details' `focalPoint` and
  /// `localFocalPoint` are normal Flutter event coordinates, not
  /// RotatableInteractiveViewer scene coordinates. See
  ...
  /// listeners are notified. If the value is set, RotatableInteractiveViewer will update
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      RotatableInteractiveViewer.getNearestPointOnLine(point, quad.point0, quad.point1),
      RotatableInteractiveViewer.getNearestPointOnLine(point, quad.point1, quad.point2),
      RotatableInteractiveViewer.getNearestPointOnLine(point, quad.point2, quad.point3),
      RotatableInteractiveViewer.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<RotatableInteractiveViewer> createState() => _RotatableInteractiveViewerState();
}

class _RotatableInteractiveViewerState extends State<RotatableInteractiveViewer> with TickerProviderStateMixin {
  ...
  Rect get _boundaryRect {
    ...
    assert(!boundaryRect.isEmpty, "RotatableInteractiveViewer's child must have nonzero dimensions.");
  ...
  @override
  void didUpdateWidget(RotatableInteractiveViewer oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child != null) {
      child = _RotatableInteractiveViewerBuilt(
        childKey: _childKey,
      ...
      // When using RotatableInteractiveViewer.builder, then constrained is false and the
      ...
      child = LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return _RotatableInteractiveViewerBuilt(
            childKey: _childKey,
            clipBehavior: widget.clipBehavior,
            constrained: widget.constrained,
            alignment: widget.alignment,
    ...
    // When pan, scale, and rotate are all off, omit [GestureDetector] so its
    // [ScaleGestureRecognizer] does not compete in the gesture arena with
    // raw [Listener] handlers on the child (e.g. live mask preview while dragging).
    final bool useScaleGestureDetector =
        widget.panEnabled || widget.scaleEnabled || widget.rotateEnabled;
    final Widget interactiveChild = useScaleGestureDetector
        ? GestureDetector(
...
// RotatableInteractiveViewer's depending on if it's using a builder or a child.
class _RotatableInteractiveViewerBuilt extends StatelessWidget {
  const _RotatableInteractiveViewerBuilt({
    required this.child,
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return RotatableInteractiveViewer.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = RotatableInteractiveViewer.getNearestPointInside(point, boundary);
