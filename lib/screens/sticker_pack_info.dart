...
class _StickerPackInfoScreenState extends State<StickerPackInfoScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (arguments == null) {
      return Scaffold(
        appBar: AppBar(
          title: Text('Sticker Pack Info'),
        ),
        body: Center(
          child: Text('No sticker pack info available.'),
        ),
      );
    }
        ...
        onPressed: () async {
          ...
          if (stickerFetchType == 'staticStickers') {
            ...
            tryImage = WhatsappStickerImageHandler.fromAsset(
                    "sticker-packs/${stickerPack.identifier}/${stickerPack.trayImageFile}")
                .path;
          } else {
            final dio = Dio();
            ...
            downloads.add(
              dio.download(
                "${BASE_URL}/sticker-packs/${stickerPack.identifier}/${stickerPack.trayImageFile}",
            ...
            for (var e in stickerPack.stickers!) {
              var urlPath =
                  "${BASE_URL}/sticker-packs/${stickerPack.identifier}/${(e.imageFile as String)}";
              var savePath =
                  ...
                  child: stickerFetchType == "remoteStickers"
                      ? FadeInImage(
                          placeholder:
                              const AssetImage("assets/images/logo.png"),
                          image: NetworkImage(
                              "${BASE_URL}/sticker-packs/${stickerPack.identifier}/${stickerPack.stickers![index].imageFile as String}"),
                        )
                      : Image.asset(
